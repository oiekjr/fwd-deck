...
Before completing any task that changes code or runtime-affecting project files, run the following commands:
...
These checks are not required after investigation-only tasks or documentation-only changes unless the documentation change also modifies executable examples, generated artifacts, configuration, commands, release behavior, or operational workflow in a way that can affect runtime behavior.
