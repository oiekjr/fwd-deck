...
`list --query` は、`name` と `description` に対して大文字小文字を区別しない部分一致検索を行います。  
...
`start` は NAME を指定しない場合に対話選択を表示します。  
...
`recover` は、NAME を省略した場合に状態ファイルで stale と判定された追跡中トンネルを現在の設定に基づいて再起動します。  
...
`stop` は NAME を指定しない場合に対話選択を表示します。  
...
同じ `name` がグローバル設定とローカル設定の両方に存在する場合、対話実行時は編集対象を選択します。  
